...
const updateProposal = async(req, res) => {
    ...
    if (singlePoll != null){
        ...
        await PollModel.create({proposalId : req.query.proposal_id, address : req.body.address, poll: 0}) //反対は、0で表している
        const singleProposal = await ProposalModel.findOne({_id : req.query.proposal_id})
        singleProposal.against = singleProposal.against + 1
        await ProposalModel.updateOne({_id: req.query.proposal_id}, singleProposal)
        return res.status(200).json({message: "反対に投票成功"}) 
    }
}
